/**
 * 树形表格 2.x date:2019-11-08 License By http://easyweb.vip
 */
layui.define(['layer', 'laytpl', 'form'], function (exports) {
    ...
    $.ajax({
        url: layui.cache.base + 'lib/extend/treeTable/treeTable.css',
        async: false,
    ...
    TreeTable.prototype.bindEvents = function () {
        ...
        // 列宽拖拽调整
        /*
		 * $view.off('mousedown.resize').on('mousedown.resize', '.ew-tb-resize',
		 * function (e) { layui.stope(e); var index =
		 * $(this).parent().data('index'); $(this).data('move', 'true');
		 * $(this).data('x', e.clientX); var w =
		 * $(this).parent().parent().parent().parent().children('colgroup').children('col').eq(index).attr('width');
		 * $(this).data('width', w); });
		 * $view.off('mousemove.resize').on('mousemove.resize', '.ew-tb-resize',
		 * function (e) { layui.stope(e); var move = $(this).data('move'); if
		 * ('true' == move) { var x = $(this).data('x'); var w =
		 * $(this).data('width'); var index = $(this).parent().data('index');
		 * var nw = parseFloat(w) + e.clientX - parseFloat(x);
		 * $(this).parent().parent().parent().parent().children('colgroup').children('col').eq(index).attr('width',
		 * nw); } }); $view.off('mouseup.resize').on('mouseup.resize',
		 * '.ew-tb-resize', function (e) { layui.stope(e); $(this).data('move',
		 * 'false'); }); $view.off('mouseleave.resize').on('mouseleave.resize',
		 * '.ew-tb-resize', function (e) { layui.stope(e); $(this).data('move',
		 * 'false'); });
		 */
